...
`lambda`默认不能改变值捕获的变量。因此在默认情况下，==由lambda产生的类中的函数调用运算符是一个const成员函数==。如果`lambda`被声明为可变的，则调用运算符就不再是`const`函数了。
...
不能直接将重载函数的名字存入`function`类型的对象中，这样做会产生二义性错误。==消除二义性的方法是使用lambda或者存储函数指针而非函数名字==。
